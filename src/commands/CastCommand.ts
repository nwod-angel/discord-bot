...
import { calculateSpellcastingFactors, SpellcastingFactorsConfig } from "../spellcasting/SpellcastingFactorsCalculator.js";
...
export const CastCommand: Command = {
    ...
    run: async (client: Client, interaction: CommandInteraction) => {
        ...
        const calculatorConfig: SpellcastingFactorsConfig = {
            action: spell.action as 'instant' | 'extended',
            potency: spell.potency,
            targets: spell.targets,
            size: spell.size,
            radius: spell.radius,
            radius_advanced: spell.radius_advanced,
            volume: spell.volume,
            volume_advanced: spell.volume_advanced,
            duration_turns: spell.duration_turns,
            duration_hours: spell.duration_hours,
            duration_days: spell.duration_days,
            duration_advanced_prolonged: spell.duration_advanced_prolonged as string | undefined,
        };

        const { factors, total } = calculateSpellcastingFactors(calculatorConfig);

        spell.dicePoolModifiers = factors;
        spell.dicePoolModifiersTotal = total;
        spell.dicePoolDescription = factors.map(dpm => `\`(${dpm.modifier})\` ${dpm.type} ${dpm.value}`).join('\n')
        ...
        switch (spell.action) {
            case 'instant':
                embed.setTitle(`Instant spellcasting factors mod = ${spell.dicePoolModifiersTotal}`)
                break
            case 'extended':
                embed.setTitle(`Extended spellcasting factors target = ${spell.dicePoolModifiersTotal}`)
                break
        }
